...
public:
    Wheel(const KDL::Tree& tree,
          ...
          const std::shared_ptr<Joint> steer_joint,
          const std::shared_ptr<Joint> axle_joint,
          const double circ);
...
private:
    ...
    std::shared_ptr<Joint> steer_joint_;    // Steering joint
    std::shared_ptr<Joint> axle_joint_;
...
// by joint_name.
std::shared_ptr<Joint> getJoint(const string& joint_name, const bool is_steer_joint,
                           const NodeHandle& ctrlr_nh,
                           ...
                           VelocityJointInterface *const vel_joint_iface)
{
    if (eff_joint_iface != NULL)
    {
        ...
        if (handle_found)
        {
            ...
            std::shared_ptr<Joint> joint(new PIDJoint(handle, urdf_joint, ctrlr_nh));
            return joint;
        }
    }
    ...
    if (pos_joint_iface != NULL)
    {
        ...
        if (handle_found)
        {
            ...
            std::shared_ptr<Joint> joint(new PosJoint(handle, urdf_joint));
            return joint;
        }
    }
    ...
    if (vel_joint_iface != NULL)
    {
        ...
        if (handle_found)
        {
            ...
            std::shared_ptr<Joint> joint(new VelJoint(handle, urdf_joint));
            return joint;
        }
    }
     ...
     NodeHandle& ctrlr_nh)
{
    ...
    for (int i = 0; i < wheel_param_list.size(); i++)
    {
        ...
        const std::shared_ptr<const urdf::Joint> steer_joint =
